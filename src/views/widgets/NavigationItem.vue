<template>
  ...
  >
    ...
    <template #title>
      <i
        v-if="calcShouldRenderIcon('submenu', route)"
        :class="calcIcon(route)"
      ></i>
    ...
    <template v-for="child in route.children">
      ...
      >
        <i
          v-if="calcShouldRenderIcon('childItem', child)"
          :class="calcIcon(child)"
        ></i>
  ...
  <el-menu-item v-else :index="route.name" @click="nodeClick(route)">
    <i
      v-if="calcShouldRenderIcon('menuItem', route)"
      :class="calcIcon(route)"
    ></i>
    <span>{{ calcTitle(route) }}</span>
  </el-menu-item>
  ...
  methods: {
    calcTitle(route) {
      if (route.meta && route.meta.title) {
        return this.$i18n.te(route.meta.title)
          ? this.$i18n.t(route.meta.title)
          : route.meta.title;
      } else {
